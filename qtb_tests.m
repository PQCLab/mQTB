...
tests.rps.seed = 161;
tests.rps.nsample = 10.^([2,3,4,5,6] + max(0,N-3));
tests.rps.nexp = 1000;
tests.rps.rank = 1;
...
tests.rmspt_2.nsample = 10.^([2,3,4,5,6] + max(0,N-2));
...
tests.rmspt_d.nsample = 10.^([2,3,4,5,6] + (N-1));
...
tests.rnp.nsample = 10.^([2,3,4,5,6] + (N-1));
tests.rnp.nexp = 1000;
tests.rnp.rank = Dim;
tests.rnp.generator = {'haar_purified', 'rank', 1, 'init_err', {'unirnd', 0, 0.05}, 'depol', {'unirnd', 0, 0.01}};
